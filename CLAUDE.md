...
- All page data: `next: { revalidate: 86400 }` (24 h ISR) + tags `page:{path}`, `pages`, `routes`. Magazine queries tag `magazine-issues` / `magazine-issue:{slug}`. Document repository queries tag `documents`. Site settings query tags `site-settings`. Theme query tags `routes`.
- `app/api/revalidate/route.ts` — Strapi webhook target (secret via `x-revalidate-secret` header, `body.secret` fallback). Uses `revalidateTag(tag, { expire: 0 })` for immediate expiry. Model mapping: `route` → `routes`; `page` → `pages` + `page:{path}`; `magazine-issue` → `magazine-issues` + `magazine-issue:{slug}`; `document` / `document-category` → `documents`; `site-setting` → `site-settings`; **any other model** → `pages` + `routes` (footer, global-theme, block-group, staff, organizational-unit and form all ride inside the unified page query, so every page fetch must expire). The Strapi-side webhook is configured in the CMS admin — it must have create/update/delete/publish/unpublish entry events enabled for ALL content types that feed rendering.
...
- **Editions live at the dedicated route `/revista/{slug}`** (`app/revista/[slug]/page.tsx`) — public by design (only `conversionStatus: "ready"` + published), no visibility/role inheritance. The archive block's `publications` selection only governs what each archive *lists*, not who can open an edition URL. The static segment shadows the catch-all; **`revista` is a reserved path segment** (with `magazine`, `articulos`, `login`, `api`, `pagina`, `_next`) — the CMS `route` content type rejects paths whose first segment is reserved (lifecycle validation in `foues-cms-api`).
- Back navigation is `BackLink` (client): `history.back()` when the referrer is same-origin, else fallback to `/` — editions are reachable from any page holding an archive block, so there is no fixed back target.
...
- The archive grid is the `blocks.magazine-archive` SDUI block — a self-fetching async RSC (no fields from the page query). The RSC resolves cover URLs and hands serialisable data to `MagazineArchiveClient`, which owns a client-side search box (title/edition number, accent-insensitive via `lib/search.ts`).
...
- `getDocumentRepositoryData()` (`lib/strapi.ts`) fetches categories + published documents WITHOUT role filtering (cached, tag `documents`) — role gating happens per request in JS against the session role. Never bake the role filter into the cached fetch.
- Rendering is split: the RSC resolves the role gate and hands serialisable data to `DocumentRepositoryClient` (search box + sections). **Gate invariant:** upload-only categories (`canRead` false) are sent with an EMPTY `documents` array — their list data must never reach the browser. The search only narrows already-loaded lists (title or category name, accent-insensitive), so it cannot widen access.
- **Category visibility vs. document-list visibility are two different rules.** A category section renders when the session can either READ it (`canReadCategory()`, `lib/document-upload-rule.ts`: `allowed_roles` empty → any logged-in user, else role ∈ `allowed_roles`, same rule as `filterByVisibility` in the Navbar) OR upload to it (`canUploadToCategory()`) — otherwise an upload-only role (in `upload_roles` but not `allowed_roles`) could never reach its own upload form. The documents list itself still only renders for sessions that pass the read rule; upload-only sessions see a short note instead. Download gating in the proxy route stays read-rule-only, unaffected by this.
...
- **Stage 2 — site uploads (one-time ticket, direct browser→CMS transfer).** `app/api/documents/upload-ticket/route.ts` validates session + role (`getDocumentCategoryForUpload()` + `canUploadToCategory()`, same as before) and calls the CMS's `POST /documents/upload-tickets` to mint a short-lived, single-use ticket. The browser then uploads the file directly to the CMS via XHR (`DocumentUploadForm.tsx`) against the returned `uploadUrl` (the CMS's public, ticket-gated endpoint) — this app never touches file bytes on either hop anymore. `mapUploadError`/`lib/document-upload-messages.ts` is shared by both the issuance route and the client for CMS error-code mapping.
- The CMS side requires `SITE_ORIGIN` configured for CORS, since the browser now calls the CMS's origin directly instead of going through this app — see `foues-cms-api`.
- **Site settings.** `getSiteSettings()` (`lib/strapi.ts`) fetches the single type `site-setting` (GraphQL field `max_upload_mb`), cache tag `site-settings`, falling back to `{ maxUploadMb: 15 }` on any failure and clamping to `[1, 500]` even on success. Used here only for UI messaging (the form's "PDF, máximo N MB" hint and the client-side pre-check in `DocumentUploadForm.tsx`) — this app no longer sits on the file-bytes path at all, so it enforces nothing itself. The real ceiling chain now lives entirely on the CMS side: `resolveMaxUploadMb` (`src/api/document/controllers/upload.ts`, same `[1, 500]` clamp) → formidable's `maxFileSize` (`../foues-cms-api/config/middlewares.ts`, `strapi::body` → `config.formidable.maxFileSize`) → the reverse proxy's `client_max_body_size` on the **`CMS_DOMAIN`** server block (`../foues-cms-api/nginx/templates/default.conf.template`) — the browser uploads directly there now, so the `FRONTEND_DOMAIN` block's limit is no longer in this path. A 500 MB ceiling also means the CMS host needs enough free disk for concurrent large uploads' temp files plus permanent storage. `DocumentRepository.tsx` calls `getSiteSettings()` once per render and passes `maxUploadMb` down as a prop to `DocumentUploadForm.tsx` (never fetched client-side).
...
- **noindex rules**: non-public routes (visibility `requires-login` or `allowed_roles` non-empty) are ALWAYS `robots: noindex, nofollow`, regardless of CMS SEO settings; `/pagina/{n}` virtual paths inherit the parent route's gate. Magazine editions (`/revista/{slug}`) are public and indexable (still excluded from the sitemap in v1).
...
8. **Token-holding proxy routes** (`/api/magazine-track`, `/api/documents/[documentId]`, `/api/articles/search`) so restricted Strapi tokens never reach the browser. Article search is public-content-only (containsi on title), rate-limited in-memory per IP in the route itself (proxy.ts only covers Server Actions), because path-based `/pagina/{n}` pagination means client-side filtering would only see the visible page.
