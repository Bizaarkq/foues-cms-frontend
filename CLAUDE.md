...
- **Stage 2 — site uploads.** `app/actions/submit-document.ts` (`submitDocument`, submit-form trust model): re-fetches the category via `getDocumentCategoryForUpload()` (REST, `env.documentToken`, `cache: "no-store"` — always fresh, never Next's data cache) and re-validates with `canUploadToCategory()` (`lib/document-upload-rule.ts`, the single source of truth for the rule: `uploadEnabled` AND (`uploadRoles` non-empty ? role ∈ `uploadRoles` : falls back to the read rule)), then validates the file (PDF mime + extension + `%PDF-` magic bytes + the CMS-configured size cap, see "Site settings" below) before forwarding multipart to the CMS `POST /api/documents/upload`. `DocumentUploadForm.tsx` (client, `useActionState`) renders per category in `DocumentRepository.tsx` only when the same rule passes for the session (UI visibility only — the action is authoritative). `next.config.ts` sets `experimental.serverActions.bodySizeLimit: "16mb"` (default is 1 MB) to admit the 15 MB PDF ceiling plus multipart overhead. **Accepted v1 residual risks**: no upload idempotency key, so a client-side timeout/network error can leave a duplicate document if the user retries after a request that actually succeeded server-side (the error message tells them to check the list first); and rate limiting of these (up to 16 MB) Server Action bodies is per-IP only (`proxy.ts`), same as every other Server Action.
- **Site settings.** `getSiteSettings()` (`lib/strapi.ts`) fetches the single type `site-setting` (GraphQL field `max_upload_mb`), cache tag `site-settings`, falling back to `{ maxUploadMb: 15 }` on any failure and clamping to `[1, 15]` even on success. 15 MB is a hard ceiling, not just a default: `next.config.ts`'s `serverActions.bodySizeLimit: '16mb'` is a Next.js build-time setting (that file deliberately imports no env/CMS config so Docker builds work without secrets), so the CMS value can only lower the effective upload limit — raising it past 15 MB needs a rebuild. `DocumentRepository.tsx` calls it once per render and passes `maxUploadMb` down as a prop to `DocumentUploadForm.tsx` (never fetched client-side).
